...
function recupererMessages(aTrouver, groupBy)
{
    return new Promise((resolve, reject) => {
        MongoClient.connect(urlMongodb,{ useUnifiedTopology: true } ,function(err, db) {
            ...
            // ajout donnees d'aggregation
            let aggregationRequete = []
            if (Object.keys(aTrouver).length > 0) {
                aggregationRequete.push(aTrouver)
            }
            if (Object.keys(groupBy).length > 0) {
                aggregationRequete.push(groupBy)
            }

            dbMongo.collection('messages').aggregate(aggregationRequete).toArray(function(err, res) {
